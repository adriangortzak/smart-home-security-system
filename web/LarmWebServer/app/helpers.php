...
//    $userGroup = App\User::where('email', Auth::user()->email)->first()->group;
	...
	echo '<span style="display:inline" id="'. $user->id .'-groupTag" class="badge bg-info">'. App\groups::where('id',$user->group)->first()->name .'</span>';
	echo '<select style="display:none" id="'. $user->id.'-userGroup">';
	echo getGroupOptions($user->group);
	echo '</select>';
	echo '<div class="pull-right hidden-phone">';
	echo '<button id="'. $user->id.'-groupUpdateButton" style="display:none" onclick="updateGroup(' . $user->id . ')" class="btn btn-success btn-xs"><i class="fa fa-check "></i></button>';
	echo '<button id="'. $user->id.'-groupCancelButton" style="display:none" onclick="cancelGroupEdit(' . "'" .$user->id . "'" . ')" class="btn btn-danger btn-xs"><i class="fa fa-times "></i></button>';
	echo '<button id="'. $user->id.'-groupEditButton" style="display:inline" onclick="changeGroup(' . "'" .$user->id . "'" . ')" class="btn btn-primary btn-xs"><i class="fa fa-pencil "></i></button>';
	echo '<button id="'. $user->id.'-removeUserButton" style="display:inline" onclick="removeUser(' . "'" .$user->email . "'" . ')" class="btn btn-danger btn-xs"><i class="fa fa-trash-o "></i></button>';
        echo '</div>';
        echo '</div>';
	echo '</li>';
    }
