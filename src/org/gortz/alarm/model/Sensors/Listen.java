    ...
    public void listen(){
        ...
        RawEventListener rel = new RawEventListener() {
            ...
            public void eventReceived(RawEvent rawEvent) {
                ...
                String regexPattern = "class:(?<type>command|sensor).*protocol:(?<protcol>\\w+);model:(?<model>\\w+);(?:house:(?<house>\\d+);unit:(?<unit>\\d);)?(?:group:(?<group>\\d);)?(?:code:(?<code>\\w+);)?method:(?<method>[a-z]+);";
                Pattern pattern = java.util.regex.Pattern.compile(regexPattern);
                Matcher matcher = pattern.matcher(temp);
                if(matcher.matches()){
                    CommandObject c = new CommandObject(matcher.group(2),matcher.group(3),matcher.group(4),matcher.group(5),matcher.group(6),matcher.group(7), matcher.group(8));
